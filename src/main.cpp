...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::vec4 horizontal_w_camera = glm::vec4(x, 0.0f, z, 0.0f);

        // W
        if (g_WPressed)
        {
            cam_move += -horizontal_w_camera * speed * delta_t;
        }
        // A
        if (g_APressed)
        {
            cam_move += -u_camera * speed * delta_t;
        }
        // S
        if (g_SPressed)
        {
        ...
        camera_position_c  = glm::vec4(x_fixed,y_fixed,z_fixed,1.0f) + cam_move;
        camera_view_vector = glm::vec4(-x,-y,-z,0.0f); // Vetor "view", sentido para onde a câmera está virada
        camera_up_vector   = glm::vec4(0.0f,1.0f,0.0f,0.0f); // Vetor "up" fixado para apontar para o "céu" (eito Y global)s
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    if (key == GLFW_KEY_W)
    {
        g_WPressed = (action == GLFW_PRESS)? true: ((action == GLFW_RELEASE)? false: g_WPressed);
    }
    // A
    if (key == GLFW_KEY_A)
    {
        g_APressed = (action == GLFW_PRESS)? true: ((action == GLFW_RELEASE)? false: g_APressed);
    }
    // S
    if (key == GLFW_KEY_S)
    {
        g_SPressed = (action == GLFW_PRESS)? true: ((action == GLFW_RELEASE)? false: g_SPressed);
    }
    // D
    if (key == GLFW_KEY_D)
    {
        g_DPressed = (action == GLFW_PRESS)? true: ((action == GLFW_RELEASE)? false: g_DPressed);
    }
